...
export function buildJewelryPrompt(analysis, template, imageCount, extraPrompt = '', presetOptions = {}) {
    ...
    const { backgroundPreset, religionPreset, dressCodePreset, hasCustomModelPhoto, consistencyMode, sizes } = presetOptions;
    ...
    // Build size information string
    let sizeInfo = '';
    if (sizes && Object.keys(sizes).length > 0) {
        const sizeEntries = Object.entries(sizes)
            .filter(([_, sizeData]) => sizeData?.value)
            .map(([index, sizeData]) => {
                const piece = analysis.pieces?.[index];
                const pieceName = piece?.type || piece?.description || `Piece ${parseInt(index) + 1}`;
                return `${pieceName}: ${sizeData.value} ${sizeData.unit}`;
            });
        
        if (sizeEntries.length > 0) {
            sizeInfo = ` EXACT SIZES — ${sizeEntries.join('; ')}. Render at these precise dimensions for accurate scale representation.`;
        }
    }
